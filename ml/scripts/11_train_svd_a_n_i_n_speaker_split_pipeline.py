...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Train an SVD pathology pipeline using only a_n and i_n with "
            "speaker-based train/validation/test splits."
        )
    )
    parser.add_argument(
        "--labels-source-root",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<subject_id>/vowels/*-a_n.wav and *-i_n.wav."
        ),
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("ml/models/svd_ai_speaker_split"),
        help="Output directory for features, metrics, split info and predictions.",
    )
    parser.add_argument(
        "--val-size",
        type=float,
        default=0.1,
        help="Validation split ratio over the full dataset.",
    )
    parser.add_argument(
        "--test-size",
        type=float,
        default=0.2,
        help="Test split ratio over the full dataset.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def collect_subject_labels(labels_root: Path) -> dict[str, int]:
    ...
    for group_dir in sorted(p for p in labels_root.iterdir() if p.is_dir()):
        ...
        for subject_dir in sorted(
            p for p in group_dir.iterdir() if p.is_dir() and p.name.isdigit()
        ):
...
) -> tuple[list[SubjectRecord], dict[str, int]]:
...
def split_subjects_three_way(
    ...
    if val_size <= 0 or test_size <= 0:
        raise SystemExit("Both --val-size and --test-size must be greater than 0.")
    if val_size + test_size >= 1.0:
        raise SystemExit("--val-size + --test-size must be less than 1.0.")
    ...
    train_subjects, holdout_subjects, train_labels, holdout_labels = train_test_split(
        unique_subjects,
        unique_labels,
...
def main() -> None:
    ...
    records, dataset_stats = build_subject_records(labels_root, wav_root, token_names)
    if not records:
        raise SystemExit(
            "No valid subjects found. Check labels_source/wav_root and ensure "
            "a_n and i_n WAV files exist for each subject."
        )
    ...
    feature_rows, feature_names = build_feature_table(records, token_names)
    ...
    train_subjects, val_subjects, test_subjects = split_subjects_three_way(
        subjects=subjects,
        labels=y,
    ...
    val_subject_ids = subjects[val_mask]
    test_subject_ids = subjects[test_mask]

    print(
        f"[INFO] Split complete: train={x_train.shape[0]} "
        f"validation={x_val.shape[0]} test={x_test.shape[0]} subjects"
    )
    ...
    print("[INFO] Model training complete. Evaluating validation and test predictions...")

    val_predictions = {name: model.predict(x_val) for name, model in models.items()}
    test_predictions = {name: model.predict(x_test) for name, model in models.items()}

    val_metrics = {
        model_name: evaluate_predictions(y_val, y_pred)
    ...
    summary = {
        "dataset": {
            **dataset_stats,
            ...
            "validation_subject_count": int(np.sum(val_mask)),
        ...
        "split_strategy": {
            "level": "speaker_subject",
            "guarantee": (
                "The same subject_id cannot appear in more than one split. "
                "All vowels/features for a speaker stay together."
            ),
            "val_size": args.val_size,
            "test_size": args.test_size,
            "seed": args.seed,
        ...
        "test_models": test_metrics,
        "paths": {
            "features_csv": portable_repo_path(out_dir / "features_subject_level.csv"),
            "validation_predictions_csv": portable_repo_path(out_dir / "validation_predictions.csv"),
            "test_predictions_csv": portable_repo_path(out_dir / "test_predictions.csv"),
            "metrics_json": portable_repo_path(out_dir / "metrics.json"),
            "split_json": portable_repo_path(out_dir / "split.json"),
        },
    }

    split_payload = {
        "train_subjects": sorted(map(str, train_subjects.tolist())),
        "validation_subjects": sorted(map(str, val_subjects.tolist())),
        "test_subjects": sorted(map(str, test_subjects.tolist())),
    }
    ...
    print(
        "[INFO] Subjects used="
        f"{len(records)} train={int(np.sum(train_mask))} "
        f"validation={int(np.sum(val_mask))} test={int(np.sum(test_mask))}"
    )
    for model_name, result in val_metrics.items():
        print(
            f"[INFO] validation/{model_name}: "
            f"accuracy={result['accuracy']:.4f} f1={result['f1_score']:.4f}"
        )
    for model_name, result in test_metrics.items():
        print(
            f"[INFO] test/{model_name}: "
            f"accuracy={result['accuracy']:.4f} f1={result['f1_score']:.4f}"
        )
